...
class TestEntityServiceFindOrCreate:
    ...
    async def test_find_or_create_company_by_name_fuzzy(self):
        ...
        mock_query = MagicMock()
        mock_query.filter.return_value = mock_query
        # First call for domain match (no match), second for name match
        mock_query.all.return_value = [existing_entity]
        db.query.return_value = mock_query

        identifiers = {"name": "Acme Corp"}  # fuzzy match to "Acme Corporation"

        # "Acme Corp" vs "Acme Corporation" is ~80% — below 85% threshold,
        # so find_or_create will try to create a new entity. Patch Entity() to
        # avoid SQLAlchemy mapper validation of unrelated models.
        mock_entity = _make_entity(user_id=user_id, entity_type="company", name="Acme Corp")
        with patch("app.services.entities.entity_service.Entity", return_value=mock_entity):
