"""Add voice extraction tables: voice_sessions, extraction_templates; extend findings
...
def upgrade() -> None:
    ...
    # Add FK constraint for template_id after extraction_templates is created
    # (template_id column already created above without FK; we add FK below)
    ...
    # Now add FK from voice_sessions.template_id -> extraction_templates.id
    op.create_foreign_key(
        "fk_voice_sessions_template_id",
        "voice_sessions",
        "extraction_templates",
        ["template_id"],
        ["id"],
    ...
    # -- Extend findings with voice_session_id and user_id --
    op.add_column(
        "findings",
        sa.Column(
            "voice_session_id",
...
def downgrade() -> None:
    ...
    op.drop_column("findings", "voice_session_id")
    op.drop_constraint("fk_voice_sessions_template_id", "voice_sessions", type_="foreignkey")
